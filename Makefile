...
bin          := chs-streaming-api-backend
chs_envs     := $(CHS_ENV_HOME)/global_env $(CHS_ENV_HOME)/chs-streaming-api-backend/env
...
ifndef version
	...
	cp ./routes.yaml $(tmpdir)/routes.yaml
	cd $(tmpdir) && zip ../$(bin)-$(version).zip $(bin) start.sh routes.yaml
	rm -rf $(tmpdir)
